...
namespace _11._12._23_lab
{
    class Product
    {
        ...
        public Product(string[] productInfo, string[] sellInfo, string productName, string expiration)
        {
            ...
            this.SellInfo = sellInfo; this.ProductName = productName;
    ...
    internal class Program
    {
        ...
        {
            Product[] Warehouse = new Product[3] {new Product(new string[4] {"10.04", "09.04", "20", "100"}, new string[2] {"11.04", "10"}, "Зубная щетка", "Бессрочно"),
            new Product(new string[4] {"10.06", "09.06", "1000", "50"}, new string[2] {"12.06", "100"}, "Яблоко", "10.07"), new Product(new string[4] {"02.05", "01.05", "500", "80"}, 
            new string[2] {"03.05", "255"}, "Молоко", "05.05")};
            Console.WriteLine("Введите текущую дату:"); string currentDate = Console.ReadLine();
            Console.WriteLine("Остатки годного товара:");
            for (int i = 0; i < Warehouse.Length; i += 1)
            {
                Product product = Warehouse[i]; if (String.Compare(currentDate, Warehouse[i].Expiration) == -1)
                {
            ...
            Console.WriteLine(); Console.WriteLine("Товары:");
            for (int i = 0; i < Warehouse.Length; i += 1)
            {
                Product product = Warehouse[i]; Console.WriteLine("Продано {0} '{1}' стоимостью в {2}", product.SellInfo[1], product.ProductName, product.getIncome());
            }
            Console.WriteLine(); 
            Console.ReadLine();
        }
